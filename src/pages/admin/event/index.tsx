...
export const getServerSideProps: GetServerSideProps<Props> = async (context) => {
	...
	const query = await db.event.findMany({
		where: { userId: session?.user?.id }
	})
	// const tz = await db.$queryRaw`Select now();`
...
const Page: NextPageWithLayout<Props> = (props) => {
	...
	return (
		<div className="container mx-auto">
			...
			{/* {typeof props.eventItems[0].updated_at} */}
			...
			content.push(
				<tr className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
					...
					<td className="px-6 py-4">
						...
						{dayjs(value.created_at).format('DD/MM/YY HH:mm:ss')}
					</td>
					<td className="px-6 py-4 flex gap-x-2">
						<Link href={`/event/${value.id}`} className="font-medium text-blue-600 dark:text-blue-500 hover:underline">View</Link>
						<Link href={`/event/${value.id}/scoreboard`} className="font-medium text-blue-600 dark:text-blue-500 hover:underline">Score</Link>
						<Link href={`/admin/event/${value.id}/log`} className="font-medium text-blue-600 dark:text-blue-500 hover:underline">Log</Link>

...
function EventPage() {
	...
	const { data, isLoading } = useSWR<EventList>(`/api/event?limit=8&page=${router.query.page || 1}`)
	...
	return (
		<div className='container mx-auto'>
			...
			<div className='my-4 flex justify-center gap-x-4'>
				{ data.meta.prev && <Link href={ `/event?page=${data.meta.prev}` } className={ `"font-medium text-blue-600 dark:text-blue-500 hover:underline"` }>
					หน้าที่แล้ว
				</Link> }
				{ data.meta.next && <Link href={ `/event?page=${data.meta.next}` } className={ `"font-medium text-blue-600 dark:text-blue-500 hover:underline"` }>
					หน้าถัดไป
				</Link> }
			</div>
		</div>

...
import { db, Prisma, Event, Activity } from '@/db'
...
const Page: NextPageWithLayout = () => {
	...
	const submit = async (e: React.FormEvent<HTMLFormElement>) => {
		...
		try {
			...
			const req = await fetch(`/api/users/activity`, {
				method: "POST",
				headers: {
			...
			<form onSubmit={ submit } className={ `mt-2` } >
				...
				<div className="mb-6">
					...
					<div className="flex">
						...
						<DatePicker required onSelect={ ({ date }) => setAssignedDate(date as Date) } placeholder="Start date" timepicker className={ css["input.date"] } />
					</div>
				</div>

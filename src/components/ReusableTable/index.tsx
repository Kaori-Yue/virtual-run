...
export type CompactType = {
	headers: (HeaderOptions | string | undefined)[]
...
function ReusableTable({ headers, contents, paginate, isLoading, preload, forceLoadingWhenNoContent = true, }: CompactType) {
		...
		<div className="relative overflow-x-auto shadow-md sm:rounded-lg">
			<table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
				<thead className={ headStyleDefault }>
					<tr>
						{ headers.filter((f): f is string | HeaderOptions => f !== undefined).map((v, i) => typeof v !== 'string'

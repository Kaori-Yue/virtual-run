		...
		<nav className="bg-white border-gray-200 dark:bg-gray-900 dark:border-gray-700">
			<div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
				<Link href="/" className="flex items-center">
					...
					<span className="self-center text-2xl font-semibold whitespace-nowrap dark:text-white">Virtual Run</span>
				</Link>
				...
				<div className="hidden w-full md:block md:w-auto" id="navbar-dropdown">
					<ul className="flex flex-col font-medium p-4 md:p-0 mt-4 border border-gray-100 rounded-lg bg-gray-50 md:flex-row md:space-x-8 md:mt-0 md:border-0 md:bg-white dark:bg-gray-800 md:dark:bg-gray-900 dark:border-gray-700">
										...
										<ul className={ `group-hover:block animate-dropdown hidden right-0 z-10 absolute pt-4 py-2 w-32 xl:w-40 rounded-lg text-sm text-gray-700 dark:text-gray-400` } >
											...
											{ session.role !== 'ROOT' &&
												<li className='dark:border-gray-700 border-x bg-white dark:bg-gray-900'>
													<Link href="/profile/activity" className="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">Activity</Link>
												</li>
											}
											...
											<li className='dark:border-gray-700 pb-1 border-x border-b rounded-b-lg bg-white dark:bg-gray-900'>
												<span className='block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white'>Role: { renderRole(session.role) }</span>
											</li>
										</ul>
									</div>
						...
						<li className='!ml-4'>
							<button type='button' className='block text-gray-500 dark:text-gray-400 ' onClick={ () => setTheme(theme === 'dark' ? 'light' : 'dark') }>
								{ theme === 'dark'
									? <Sun />
									: <Moon /> }
							</button>
